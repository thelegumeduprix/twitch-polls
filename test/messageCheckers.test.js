...
describe('POLL_QUOTED_PARAMETER_DETECTION_PATTERN', function () {
  it('detects the right commands', function () {
    expect('!poll "q" "a" "b"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "q" "a" "b" "c"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "" "a" "b"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll " " "a" "b"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "  " "a" "b"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "" "a" "b" "c"').to.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('  !poll "" "a" "b" "c"').to.not.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('  !poll "" "a" "b" "c"   ').to.not.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "q" "a"').to.not.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "" "a"').to.not.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
    expect('!poll "a" ""').to.not.match(POLL_QUOTED_PARAMETER_DETECTION_PATTERN);
...
describe('POLL_QUOTED_PARAMETER_EXTRACTION_PATTERN', function () {
  it('detects the right commands', function () {
    ...
    let parameters = [...message.matchAll(POLL_QUOTED_PARAMETER_EXTRACTION_PATTERN)].map((match) => match[1]);

    expect(parameters).to.eql(['q', 'a', 'b']);

    message = '!poll "" "a" "b"';
    parameters = [...message.matchAll(POLL_QUOTED_PARAMETER_EXTRACTION_PATTERN)].map((match) => match[1]);
...
describe('POLL_VOTE_EXTRACTION_PATTERN', function () {
  it('detect and extract poll votes correctly', function () {
    expect('0').to.match(POLL_VOTE_EXTRACTION_PATTERN).and.capture(0).equals('0');

    expect('9').to.match(POLL_VOTE_EXTRACTION_PATTERN).and.capture(0).equals('9');

    expect('2').to.match(POLL_VOTE_EXTRACTION_PATTERN).and.capture(0).equals('2');
    ...
    expect('2 test').to.match(POLL_VOTE_EXTRACTION_PATTERN).and.capture(0).equals('2');
